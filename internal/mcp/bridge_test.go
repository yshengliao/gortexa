...
func rpc(t *testing.T, url, bearer string, payload map[string]any) map[string]any {
	t.Helper()
	out, _ := rpcWithSession(t, url, bearer, "", payload)
	return out
}

func rpcWithSession(t *testing.T, url, bearer, sessionID string, payload map[string]any) (map[string]any, http.Header) {
	t.Helper()
...
func initializeSession(t *testing.T, url string) string {
	t.Helper()
	init, header := rpcWithSession(t, url, "", "", map[string]any{"jsonrpc": "2.0", "id": 1, "method": "initialize"})
	if init["result"] == nil {
		t.Fatalf("initialize result missing: %v", init)
	}
	sessionID := header.Get("Mcp-Session-Id")
	if sessionID == "" {
		t.Fatalf("initialize did not return Mcp-Session-Id header")
	}
	return sessionID
}
...
func TestBridgeToolsListBeforeInitializeIsRejected(t *testing.T) {
	ts := newBridgeServer(t)

	list := rpc(t, ts.URL, "", map[string]any{"jsonrpc": "2.0", "id": 2, "method": "tools/list"})
	if list["error"] == nil {
		t.Fatalf("tools/list before initialize should return a JSON-RPC error: %v", list)
	}
}
...
func TestBridgeInitializeAndList(t *testing.T) {
	...
	list, _ := rpcWithSession(t, ts.URL, "", sessionID, map[string]any{"jsonrpc": "2.0", "id": 2, "method": "tools/list"})
...
func TestBridgeArbitrarySessionIDIsRejected(t *testing.T) {
	ts := newBridgeServer(t)

	list, _ := rpcWithSession(t, ts.URL, "", "not-a-real-session", map[string]any{"jsonrpc": "2.0", "id": 2, "method": "tools/list"})
	if list["error"] == nil {
		t.Fatalf("arbitrary session should return a JSON-RPC error: %v", list)
	}
}
...
func TestBridgeToolsCallEnforcesAuth(t *testing.T) {
	...
	call, _ := rpcWithSession(t, ts.URL, "", sessionID, map[string]any{
...
func TestBridgeToolsCallRoundTrip(t *testing.T) {
	...
	// create
	create, _ := rpcWithSession(t, ts.URL, tok, sessionID, map[string]any{
	...
	get, _ := rpcWithSession(t, ts.URL, tok, sessionID, map[string]any{
...
func TestBridgeUnknownMethod(t *testing.T) {
	ts := newBridgeServer(t)
	sessionID := initializeSession(t, ts.URL)
	resp, _ := rpcWithSession(t, ts.URL, "", sessionID, map[string]any{"jsonrpc": "2.0", "id": 9, "method": "bogus"})
	if resp["error"] == nil {
